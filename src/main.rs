...
impl TrackCardChromeWidget {
    fn new(selected: bool, favorite: bool) -> Self {
        Self {
...
fn track_card_fill(favorite: bool, theme: &ThemeTokens) -> ui::Rgba8 {
...
fn animation_requested(state: &AppState) -> bool {
    state.transport_playing
        ...
        || !matches!(state.paired_playback_guard, PairedPlaybackGuard::Idle)
}
...
fn main_output_gain(state: &AppState) -> f32 {
    let cleanup_has_paused_main = matches!(
...
fn update(state: &mut AppState, message: Message, context: &mut ui::UiUpdateContext<Message>) {
    match message {
        ...
        Message::Frame => {
            ...
            maybe_clear_paired_playback_guard_after_idle(state);
            refresh_live_spectrograms(state);
        }
...
fn paired_playback_commands_admitted(state: &mut AppState) {
    if matches!(
        state.paired_playback_guard,
        PairedPlaybackGuard::ReferenceRequired
    ) {
        state.paired_playback_guard = PairedPlaybackGuard::Active;
    }
}

fn clear_paired_playback_guard(state: &mut AppState) {
    state.paired_playback_guard = PairedPlaybackGuard::Idle;
}

fn clear_uncommitted_paired_playback_guard(state: &mut AppState) {
    if matches!(
        state.paired_playback_guard,
        PairedPlaybackGuard::ReferenceRequired
    ) {
...
fn seek_synchronized_positions(
    ...
    if let Some((reference_path, reference_duration_millis)) = reference_details {
        ...
        if !reference_was_loaded {
            ...
            if reference_transport.has_pending_load() {
                return Err(cleanup_reference_transport_failure(
                    state,
        ...
        let reference_token = match reference_transport.seek(
            state.reference_transport_generation,
...
fn seek_reference_waveform_position(
    ...
    if !state.reference_transport_loaded {
        ...
        if reference_transport.has_pending_load() {
            cleanup_reference_transport_failure(
                state,
                String::from(transport::CONTROLS_BUSY_ERROR),
            );
            context.request_repaint();
            ...
            let result = {
                ...
                load_result.and_then(|()| {
                    ...
                    if seek_result.is_ok()
                        && let Some(intent_millis) = pending_intent_millis
                    {
                        clear_pending_seek_intent_if_admitted(
                            state,
                            AuditionSource::Reference,
...
mod tests {
    ...
    fn planner_card_title_rect(
        ...
        frame
            ...
            .text_runs()
            .find_map(|run| {
                (run.text.as_str() == track_id && viewport.contains(run.rect.center()))
                    .then_some(run.rect)
            })
    ...
    fn assert_rect_close(actual: Rect, expected: Rect, tolerance: f32) {
        assert!(
            ...
            "rectangles should remain continuous: actual={actual:?}, expected={expected:?}, tolerance={tolerance}"
        );
    }
    ...
    #[test]
    fn paired_toggle_reference_failure_cleans_up_main_and_preserves_retry_state() {
        let mut state = shared_reference_playback_state();
        let error = String::from("reference play failed immediately");
        let reference_generation = state.reference_transport_generation;
        let reference_transport = state
            .reference_transport
            .as_ref()
            .expect("paired playback should have a reference transport")
            .clone();
        reference_transport.fail_next_command_for_test(error.clone());
        state.transport_position_millis = 650;
        state.review_cursor_millis = 650;
        state.reference_transport_position_millis = 1_800;
        state.loop_selections = LoopSelections {
            main: Some(LoopSelection {
                start_ratio: 0.2,
                end_ratio: 0.6,
        ...
        assert!(!state.transport_playing);
        assert!(state.transport_polling);
        assert_eq!(state.transport_position_millis, 650);
        assert_eq!(state.review_cursor_millis, 650);
        assert_eq!(state.reference_transport_position_millis, 1_800);
        assert_eq!(state.loop_selections, loops);
        assert_eq!(
            state.reference_transport_generation,
            reference_generation.wrapping_add(1)
    ...
    #[test]
    fn paired_synchronized_seek_reference_failure_cleans_up_main_atomically() {
        let mut state = shared_reference_playback_state();
        let error = String::from("reference seek failed immediately");
        state
            .reference_transport
            .as_ref()
            .expect("paired playback should have a reference transport")
        ...
        state.review_cursor_millis = 400;
        state.reference_transport_position_millis = 2_200;
        state.loop_selections = LoopSelections {
            main: Some(LoopSelection {
                start_ratio: 0.1,
                end_ratio: 0.5,
        ...
        assert_eq!(state.transport_position_millis, 400);
        assert_eq!(state.review_cursor_millis, 400);
        assert_eq!(state.reference_transport_position_millis, 2_200);
        assert_eq!(state.loop_selections, loops);
        assert!(!state.reference_transport_loaded);
        assert!(matches!(
            state.paired_playback_guard,
    ...
    fn async_paired_reference_failure_stops_main_and_preserves_root_error() {
        ...
        state.transport_position_millis = 720;
        state.review_cursor_millis = 720;
        state.reference_transport_position_millis = 2_640;
        state.loop_selections.reference = Some(LoopSelection {
            start_ratio: 0.2,
            end_ratio: 0.7,
        });
        ...
        reference_transport.set_error_for_test(
            ...
            String::from("async reference failure"),
        );
        let mut context = ui::UiUpdateContext::default();

        update(&mut state, Message::Frame, &mut context);

        assert_eq!(state.status, "async reference failure");
        assert!(!state.transport_playing);
        assert_eq!(state.transport_position_millis, 720);
        assert_eq!(state.review_cursor_millis, 720);
        assert_eq!(state.reference_transport_position_millis, 2_640);
        assert_eq!(
            state.reference_transport_generation,
            reference_generation.wrapping_add(1)
        );
    ...
    #[test]
    fn starting_reference_alongside_main_failure_stops_main_atomically() {
        let mut state = shared_reference_playback_state();
        state.transport_playing = true;
        state.transport_position_millis = 300;
        state.reference_transport_position_millis = 1_200;
        let error = String::from("reference start failed immediately");
        state
            .reference_transport
            .as_ref()
            .expect("paired playback should have a reference transport")
    ...
    fn paired_cleanup_retries_full_main_queue_without_replacing_root_error() {
        ...
        state.transport.set_command_queue_size_for_test(31);
        let mut context = ui::UiUpdateContext::default();
        update(&mut state, Message::Frame, &mut context);
        assert_eq!(state.status, root_error);
        let pause_token = state
            .transport_waiting_token
            .expect("cleanup should retry the main pause");
        assert!(!state.transport_playing);

        state.transport.set_snapshot_for_test(Snapshot {
            generation: state.transport_generation,
            acknowledged_token: pause_token,
            position_millis: state.transport_position_millis,
            playing: false,
            ready: true,
        });
        acknowledge_reference_unload(&state);
    ...
    #[test]
    fn reference_only_failure_does_not_touch_main_transport_state() {
        let mut state = shared_reference_playback_state();
        state.reference_only_playback = true;
        state.transport_playing = true;
        state.transport_position_millis = 510;
        ...
        reference_transport.set_snapshot_for_test(Snapshot {
            generation: reference_generation,
            acknowledged_token: 3,
            position_millis: 1_400,
            playing: true,
            ready: true,
        });
        reference_transport
            .set_error_for_test(reference_generation, String::from("reference-only failure"));
        let mut context = ui::UiUpdateContext::default();

        update(&mut state, Message::Frame, &mut context);

        assert_eq!(state.status, "reference-only failure");
        assert!(state.transport_playing);
        assert_eq!(state.transport_position_millis, 510);
        assert_eq!(state.review_cursor_millis, 510);
        assert_eq!(state.transport_generation, main_generation);
    ...
    #[test]
    fn main_only_playback_failure_remains_unguarded() {
        let mut state = main_only_loop_state();
        let error = String::from("main-only play failed immediately");
        state.transport.fail_next_command_for_test(error.clone());
        let mut context = ui::UiUpdateContext::default();
    ...
    fn assert_normalized_pending_comment_intents_clear_after_admission(
        main_intent_millis: u64,
        reference_intent_millis: u64,
    ) {
        let mut state = shared_reference_playback_state();
        state.loop_selections = LoopSelections {
            main: Some(LoopSelection {
        ...
        let loops = state.loop_selections;
        state.pending_main_seek_intent = Some(main_intent_millis);
        state.pending_reference_seek_intent = Some(reference_intent_millis);
        let mut context = ui::UiUpdateContext::default();
    ...
    #[test]
    fn source_start_clears_normalized_main_and_reference_intents_after_seek() {
        let mut state = shared_reference_playback_state();
        state.loop_selections = LoopSelections {
            main: Some(LoopSelection {
                start_ratio: 0.1,
        ...
        state.pending_main_seek_intent = Some(2_000);
    ...
    #[test]
    fn exact_duration_pending_comment_intents_clear_after_normalized_seek() {
        assert_normalized_pending_comment_intents_clear_after_admission(2_000, 4_000);
    }

    #[test]
    fn beyond_duration_pending_comment_intents_clear_after_normalized_seek() {
        assert_normalized_pending_comment_intents_clear_after_admission(2_500, 4_500);
    }

    #[test]
    fn normalized_pending_comment_intents_survive_busy_admission() {
        let mut state = shared_reference_playback_state();
        state.loop_selections = LoopSelections {
            main: Some(LoopSelection {
                start_ratio: 0.1,
        ...
        state.pending_main_seek_intent = Some(2_000);
        state.pending_reference_seek_intent = Some(4_000);
        state
            .reference_transport
            .as_ref()
            .expect("paired state should have a reference transport")
        ...
        assert_eq!(state.status, transport::CONTROLS_BUSY_ERROR);
        assert_eq!(state.pending_main_seek_intent, Some(2_000));
        assert_eq!(state.pending_reference_seek_intent, Some(4_000));
        assert!(state.loop_selections.main.is_some());
        assert!(state.loop_selections.reference.is_some());
    }

    #[test]
    fn normalized_pending_comment_intents_survive_failed_admission() {
        let mut state = shared_reference_playback_state();
        state.loop_selections = LoopSelections {
            main: Some(LoopSelection {
                start_ratio: 0.1,
        ...
        state
            .transport
            .fail_next_command_for_test(String::from("main seek failed"));
        let mut context = ui::UiUpdateContext::default();

        update(&mut state, Message::TogglePlayback, &mut context);

        assert_eq!(state.status, "main seek failed");
        assert_eq!(state.pending_main_seek_intent, Some(2_000));
        assert_eq!(state.pending_reference_seek_intent, Some(4_000));
    }
    ...
    fn planner_scroll_state(cards_per_stage: usize) -> AppState {
        ...
        state.library.tracks = stages
            ...
            .flat_map(|(stage_index, stage)| {
                (0..cards_per_stage).map(move |card_index| {
                    let index = stage_index * cards_per_stage + card_index;
                    let mut track = audition_track(&format!("planner-card-{index}"));
                    track.stage = stage;
                    track
                })
            })
            .collect();
        state.library.planner_order = state
            .library
            .tracks
            .iter()
            .map(|track| track.id.clone())
    ...
    fn planner_runtime_preserves_keyed_scroll_across_drag_policy_switch() {
        ...
        assert_rect_close(
            *active_layout
                .viewport_bounds
                .get(&production_scroll_id)
                .expect("the keyed ordinary scroll viewport should remain mounted"),
            production_viewport,
            0.5,
        );
        assert_rect_close(
    ...
    fn library_track_cards_paint_persistent_rails_and_stronger_outlines() {
        ...
        let mut selected = audition_track("selected-track");
        ...
        let card_fills = frame
            .paint_plan
            .fill_polygons()
            .filter(|fill| is_track_card_polygon(&fill.points))
            .collect::<Vec<_>>();
        assert_eq!(
            card_fills.len(),
            2,
        ...
        let unselected_card_points = card_fills
            ...
            .points
            .clone();
        let selected_card_fill = card_fills
            .iter()
            .find(|fill| fill.widget_id == selected_card_widget_id)
            .expect("selected card should have a polygon fill");
        let unselected_card_fill = card_fills
            .iter()
            .find(|fill| fill.widget_id == unselected_card_widget_id)
            .expect("unselected card should have a polygon fill");
        assert_eq!(
            selected_card_fill.color,
            super::track_card_fill(true, &theme),
            "favorite card should use the lighter favorite fill"
        );
        assert_eq!(
            unselected_card_fill.color,
            super::track_card_fill(false, &theme),
            "non-favorite card should retain the baseline fill"
        );
        assert_ne!(
            selected_card_fill.color, unselected_card_fill.color,
            "favorite and non-favorite card fills should differ"
        );
        assert!(
            selected_card_fill.color.r > unselected_card_fill.color.r
                && selected_card_fill.color.g > unselected_card_fill.color.g
                && selected_card_fill.color.b > unselected_card_fill.color.b,
            "favorite card fill should be lighter in every RGB channel"
        );
